...
- (void)OkAction{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        ...
        if (result) {
            hud.labelText = [NSString stringWithFormat:@"检测完成,正在备份(%@)...",[CCFileManager fileSizeString:self.path]];
        }else{
        ...
        //备份工程
        //有后缀的文件名
        NSString *tempFileName=[CCFileManager getFileNameFromFilePath:self.path];
        
        //无后缀的文件名
        NSString *fileName=[CCFileManager getFileNameNoPathComponentFromFilePath:self.path];
        ...
        if([CCFileManager fileExistsAtPath:newFilePath]){
            [CCFileManager removeItemAtPath:newFilePath];
        }
        
        result=[CCFileManager copyItemAtPath:self.path toPath:newFilePath];
        ...
        // 处理耗时操作的代码块...
        [[CCStroyBoardToPureHandProject new] transformProjectToPureHandProject:self.path];
...
- (BOOL)checkProject{
    ...
    NSArray *fileArr=[CCFileManager subPathFileArrInDirector:self.path hasPathExtension:@[@".m"]];
...
- (void)importAction{
    ...
    NSString *macDesktopPath=[CCFileManager getMacDesktop];
    macDesktopPath = [macDesktopPath stringByAppendingPathComponent:@"代码助手.m"];
    [CCFileManager createFileAtPath:macDesktopPath];
    NSString *Msg=@"文件已经生成在桌面,名字为\"代码助手.m\",请填写要转换的工程路径";
    [CCAlertAction alertWithTitle:@"导入工程" withMsg:Msg addToViewController:self withCancleBlock:nil withOkBlock:^{
        
        NSString *path=[NSString stringWithContentsOfFile:macDesktopPath encoding:NSUTF8StringEncoding error:nil];
        
        if ([CCFileManager fileExistsAtPath:path]==NO) {
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [CCAlertAction alertWithMsg:@"路径不存在!请重新填写!" addToViewController:self ActionSheet:NO];
            });
            return ;
        }
        
        NSArray *fileArr=[CCFileManager subPathFileArrInDirector:path hasPathExtension:@[@".storyboard"]];
        
        if(fileArr.count==0){
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [CCAlertAction alertWithMsg:@"工程路径不存在storyboard文件!请重新填写工程路径!" addToViewController:self ActionSheet:NO];
            });
            return;
        }
        ...
        if (self.dataArr.count>0) {
            self.path=path;
            [self.importButton setTitle:[CCFileManager getFileNameNoPathComponentFromFilePath:path] forState:(UIControlStateNormal)];
            ...
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [CCAlertAction alertWithMsg:@"工程路径不存在相关的storyboard文件!请重新填写工程路径!" addToViewController:self ActionSheet:NO];
            });
        }
