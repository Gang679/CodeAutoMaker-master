...
- (NSMutableArray *)dataArr{
	if (!_dataArr) {
        ...
        [_dataArr addObjectsFromArray:[CCSaveDataToFMDB selectDataWithIdentity:@"StroyBoardCreatePropertyCellModel"]];
	}
	return _dataArr;
}
...
- (void)addProjectAction{
    ...
    [CCAlertAction alertWithTitle:@"添加新的工程" withMsg:Msg addToViewController:self withCancleBlock:nil withOkBlock:^{
        ...
        if(fileArr.count==0){
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [CCAlertAction alertWithMsg:@"工程路径不存在storyboard文件!请重新填写!" addToViewController:self ActionSheet:NO];
            });
            return;
        }
        ...
        if (fileArr.count>0) {
            [CCSaveDataToFMDB insertDataWithData:self.dataArr WithIdentity:@"StroyBoardCreatePropertyCellModel"];
        }
...
- (NSArray<UITableViewRowAction *> *)tableView:(UITableView *)tableView editActionsForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    UITableViewRowAction *backUpRowAction=[UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDefault title:@"备份路径" handler:^(UITableViewRowAction * _Nonnull action, NSIndexPath * _Nonnull indexPath) {
        ...
        //有后缀的文件名
        NSString *tempFileName=[CCFileManager getFileNameFromFilePath:model.title];
        
        //无后缀的文件名
        NSString *fileName=[CCFileManager getFileNameNoPathComponentFromFilePath:model.title];
        ...
        [CCAlertAction alertWithTitle:Msg withMsg:nil addToViewController:self withOkBlock:nil ActionSheet:NO];
    }];
