...
@interface GetIBViewController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation GetIBViewController
...
- (void)viewDidLoad{
    ...
    [self.timer fire];
    [TabBarAndNavagation setLeftBarButtonItemTitle:@"使用简介" TintColor:[UIColor orangeColor] target:self action:@selector(helpAction)];
    [TabBarAndNavagation setRightBarButtonItemTitle:@"更多功能" TintColor:[UIColor orangeColor] target:self action:@selector(moreFuncAction)];
    
    [self check];//检测是不是安装到手机上了
}

- (void)check{
    NSString *mainPath=[CCFileManager getMacHomeDirectorInIOS];
    mainPath=[mainPath stringByAppendingPathComponent:@"Desktop"];
    if ([CCFileManager fileExistsAtPath:mainPath]==NO) {
        self.promoteLabel.text=@"请在模拟器上运行";
        self.promoteLabel.backgroundColor=[UIColor orangeColor];
    }
}
...
- (void)loadData{
    ...
    NSArray *filesArr=[CCFileManager contentsOfDirectoryAtPath:mainPath];
