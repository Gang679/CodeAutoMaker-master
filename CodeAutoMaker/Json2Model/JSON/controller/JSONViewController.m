...
- (IBAction)creatAction:(id)sender {
    ...
    //加载一些初始化数据
    NSString *macPath=[CCFileManager getMacDesktop];
    NSString *fileDirectory=[CCStroyBoardFileManager getCurDateString];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section==0) {
        ...
        if ([CCFileManager fileExistsAtPath:macDesktopPath]==NO) {
...
- (IBAction)sureAction:(id)sender{
    ...
    if([_promoteLabel.text rangeOfString:@"json"].location!=NSNotFound){
        ...
        //检验这个plist文件路径
        if ([CCFileManager fileExistsAtPath:self.dataTextView.text]==NO) {
    ...
    if ([_promoteLabel.text rangeOfString:@"url"].location!=NSNotFound) {
        [CCAlertAction alertWithTitle:@"请求方式" withMsg:nil addToViewController:self withCancleBlock:^{
            //GET
            self.isPost=NO;
            for (JSONSourceCellModel * JSONSourceModel in self.dataArr[0]) {
