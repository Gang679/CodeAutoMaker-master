...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    CCCollectionView *cccollectionView;
    tableViewContainColloectionView *tableViewContain;
    
    if (indexPath.row==0) {
        cctableView=[CCTableView new];
        [cctableView creatFatherFile:@"代码助手" andData:@[@"最大文件夹名字",@"ViewController的名字",@"自定义Cell,以逗号隔开",@"是否需要对应的Model 1:0 (不填写么默认为否)",@"是否需要对应的StroyBoard 1:0 (不填写么默认为否)",@"自定义cell可编辑(删除) 1:0 (不填写么默认为否)"]];
    }else if (indexPath.row==1) {
        cccollectionView=[CCCollectionView new];
        [cccollectionView creatFatherFile:@"代码助手" andData:@[@"最大文件夹名字",@"ViewController的名字",@"自定义Cell,以逗号隔开",@"是否需要对应的Model 1:0 (不填写么默认为否)",@"是否需要对应的StroyBoard 1:0 (不填写么默认为否)"]];
    ...
    [CCAlertAction alertWithTitle:model.title withMsg:Msg addToViewController:self withCancleBlock:nil withOkBlock:^{
        
        if (indexPath.row==0) {
            [cctableView Begin:@"代码助手" toView:self.view];
        }else if (indexPath.row==1) {
            [cccollectionView Begin:@"代码助手" toView:self.view];
        }else if (indexPath.row==2) {
