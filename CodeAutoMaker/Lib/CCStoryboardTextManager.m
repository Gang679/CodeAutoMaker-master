...
//
#import "CCStoryboardTextManager.h"
#import "CCStroyBoardFileManager.h"
#import "CCNSString.h"

static NSMutableDictionary *CCStoryboardDicM;
static NSMutableDictionary *CCStoryboardIDDicM;

@implementation CCStoryboardTextManager
+ (NSMutableDictionary *)defaultDicM{
    //添加线程锁
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (CCStoryboardDicM==nil) {
            CCStoryboardDicM=[NSMutableDictionary dictionary];
            ...
            for (NSString *str in viewsArr) {
                [CCStoryboardDicM setValue:[NSNumber numberWithInteger:0] forKey:str];
            }
        }
    });
    return CCStoryboardDicM;
}
+ (NSMutableDictionary *)defaultIDDicM{
    //添加线程锁
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (CCStoryboardIDDicM==nil) {
            CCStoryboardIDDicM=[NSMutableDictionary dictionary];
        }
    });
    return CCStoryboardIDDicM;
}

+ (void)done{
    [CCStoryboardIDDicM removeAllObjects];
...
+ (NSInteger)getAllViewCount{
    ...
    NSInteger count=0;
    for (NSString *str in viewsArr) {
        NSNumber *num=CCStoryboardDicM[str];
...
+ (NSString *)addCustomClass:(NSString *)text needDealAdapterCell:(BOOL)needDealAdapterCell{
    ...
    for (NSInteger i=0; i<arr.count; i++) {
        ...
        if (viewIdenity.length>0) {
            ...
            NSString *tempStr=[CCNSString removeSpaceBeforeAndAfterWithString:rowStr];
...
+ (NSString *)isView:(NSString *)text{
    ...
    text=[CCNSString removeSpaceBeforeAndAfterWithString:text];
    for (NSString *str in viewsArr) {
...
+ (void)dealWith_multipliedBy:(NSMutableString *)textCode{
    NSArray *arr=[CCNSString getMidStringBetweenLeftString:@"multipliedBy(" RightString:@")" withText:textCode getOne:NO withIndexStart:0 stopString:nil];
...
+ (void)dealWith_self_tableView_collectionView:(NSMutableString *)textCode isOnlyTableViewOrCollectionView:(BOOL)isOnlyTableViewOrCollectionView{
    ...
    NSArray *tableViews=[CCNSString getMidStringBetweenLeftString:@"UITableView *" RightString:@"=" withText:textCode getOne:NO withIndexStart:0 stopString:nil];
    NSArray *collectionViews=[CCNSString getMidStringBetweenLeftString:@"UICollectionView *" RightString:@"=" withText:textCode getOne:NO withIndexStart:0 stopString:nil];
...
+ (void)dealWith_UIMapView:(NSMutableString *)textCode needInserFramework:(BOOL)needInserFramework{
    ...
    if (needInserFramework) {
        [self addCodeText:@"#import <MapKit/MapKit.h>\n#error 请导入<MapKit.framework>框架" andInsertType:CCAddCodeType_Import toStrM:textCode insertFunction:nil];
    }
...
+ (NSString *)detailSpecialCustomClassLikeCell:(NSString *)rowStr{
    ...
    customClass=[CCStroyBoardFileManager getAdapterCollectionViewCellAndTableViewCellName:customClass];
    
    NSString *newCustomClass;
    NSString *text=[CCNSString removeSpaceBeforeAndAfterWithString:rowStr];
    for (NSString *str in viewsArr) {
        if ([text hasPrefix:str]) {
            newCustomClass=str;
...
+ (NSString *)noDetailSpecialCustomClassLikeCell:(NSString *)rowStr{
    ...
    customClass=[CCStroyBoardFileManager getAdapterCollectionViewCellAndTableViewCellNameForPureHandProject:customClass];
...
+ (void)addCodeText:(NSString *)code andInsertType:(CCAddCodeType)insertType toStrM:(NSMutableString *)strM insertFunction:(NSString *)insertFunction{
    switch (insertType) {
        case CCAddCodeType_Import:
            [self addCodeToImport:code toText:strM];
            break;
        case CCAddCodeType_Interface:
            [self addCodeToFirstEnd:code toText:strM];
            break;
        case CCAddCodeType_Implementation:
            [self addCodeToImplementation:code toText:strM];
            break;
        case CCAddCodeType_InsertFunction:
            [self addCodeByReplaceFunction:code WithIdentity:insertFunction toText:strM];
            break;
        case CCAddCodeType_end_last:
            [self addCodeToLastEnd:code toText:strM];
            break;
        case CCAddCodeType_Delegate:
...
+ (void)addDelegateTableViewToText:(NSMutableString *)text{
    [self addCodeText:@"UITableViewDelegate,UITableViewDataSource" andInsertType:CCAddCodeType_Delegate toStrM:text insertFunction:nil];
}
+ (void)addDelegateCollectionViewToText:(NSMutableString *)text{
    [self addCodeText:@"UICollectionViewDelegateFlowLayout,UICollectionViewDataSource" andInsertType:CCAddCodeType_Delegate toStrM:text insertFunction:nil];
}
...
+ (void)addDelegateFunctionToText:(NSMutableString *)text withTableViews:(NSDictionary *)tableViewsDic isOnlyTableViewOrCollectionView:(BOOL)isOnlyTableViewOrCollectionView{
    ...
    if (tableViews.count==1) {
        ...
        if(isOnlyTableViewOrCollectionView&&[self hasSuffixNumber:oneTableViewName]){
            [self addCodeText:@"@property (strong, nonatomic) UITableView *tableView;" andInsertType:CCAddCodeType_Interface toStrM:text insertFunction:nil];
            [self addCodeText:@"@property (strong, nonatomic) NSMutableArray *dataArr;" andInsertType:CCAddCodeType_Interface toStrM:text insertFunction:nil];
            [self addCodeText:@"- (NSMutableArray *)dataArr{\n\
             if (!_dataArr) {\n\
             _dataArr=[NSMutableArray array];\n\
             }\n\
             return _dataArr;\n\
             }" andInsertType:CCAddCodeType_Implementation toStrM:text insertFunction:nil];
            [self addCodeText:@"self.tableView.delegate=self;\nself.tableView.dataSource=self;" andInsertType:CCAddCodeType_InsertFunction toStrM:text insertFunction:@"- (void)viewDidLoad{"];
            oneTableViewName=@"";
        }else{
            NSString *oneTableViewName_new=[self upFirstCharacter:oneTableViewName];
            [self addCodeText:[NSString stringWithFormat:@"@property (strong, nonatomic) UITableView *%@;",oneTableViewName] andInsertType:CCAddCodeType_Interface toStrM:text insertFunction:nil];
            [self addCodeText:[NSString stringWithFormat:@"@property (strong, nonatomic) NSMutableArray *dataArr%@;",oneTableViewName_new] andInsertType:CCAddCodeType_Interface toStrM:text insertFunction:nil];
            [self addCodeText:[NSString stringWithFormat:@"- (NSMutableArray *)dataArr%@{\n\
                               if (!_dataArr%@) {\n\
                               _dataArr%@=[NSMutableArray array];\n\
                               }\n\
                               return _dataArr%@;\n\
                               }",oneTableViewName_new,oneTableViewName_new,oneTableViewName_new,oneTableViewName_new] andInsertType:CCAddCodeType_Implementation toStrM:text insertFunction:nil];
            [self addCodeText:[NSString stringWithFormat:@"self.%@.delegate=self;\nself.%@.dataSource=self;",oneTableViewName,oneTableViewName] andInsertType:CCAddCodeType_InsertFunction toStrM:text insertFunction:@"- (void)viewDidLoad{"];
        }
        ...
        for (NSString *cell in cells) {
            NSString *adapterCell=[CCStroyBoardFileManager getAdapterTableViewCellName:cell];
        ...
        if(registerClassText.length>0)
            [self addCodeText:registerClassText andInsertType:CCAddCodeType_InsertFunction toStrM:text insertFunction:@"- (void)viewDidLoad{"];
        ...
        //开始插入
        [self addCodeText:strM andInsertType:CCAddCodeType_end_last toStrM:text insertFunction:nil];
    }
    else{
...
//     }\n" andInsertType:CCAddCodeType_end_last toStrM:text insertFunction:nil];
    ...
    if (collectionViews.count==1) {
        ...
        if(isOnlyTableViewOrCollectionView&&[self hasSuffixNumber:oneCollectionViewName]){
            [self addCodeText:@"@property (strong, nonatomic) UICollectionView *collectionView;" andInsertType:CCAddCodeType_Interface toStrM:text insertFunction:nil];
            [self addCodeText:@"@property (strong, nonatomic) NSMutableArray *dataArr;" andInsertType:CCAddCodeType_Interface toStrM:text insertFunction:nil];
            [self addCodeText:@"- (NSMutableArray *)dataArr{\n\
             if (!_dataArr) {\n\
             _dataArr=[NSMutableArray array];\n\
            ...
            [self addCodeText:[NSString stringWithFormat:@"self.collectionView.delegate=self;\nself.collectionView.dataSource=self;"] andInsertType:CCAddCodeType_InsertFunction toStrM:text insertFunction:@"- (void)viewDidLoad{"];
            oneCollectionViewName=@"";
            
        }else{
            NSString *oneCollectionViewName_new=[self upFirstCharacter:oneCollectionViewName];
            [self addCodeText:[NSString stringWithFormat:@"@property (strong, nonatomic) UICollectionView *%@;",oneCollectionViewName] andInsertType:CCAddCodeType_Interface toStrM:text insertFunction:nil];
            [self addCodeText:[NSString stringWithFormat:@"@property (strong, nonatomic) NSMutableArray *dataArr%@;",oneCollectionViewName_new] andInsertType:CCAddCodeType_Interface toStrM:text insertFunction:nil];
            [self addCodeText:[NSString stringWithFormat:@"- (NSMutableArray *)dataArr%@{\n\
                               if (!_dataArr%@) {\n\
                               _dataArr%@=[NSMutableArray array];\n\
                               }\n\
                               return _dataArr%@;\n\
                               }",oneCollectionViewName_new,oneCollectionViewName_new,oneCollectionViewName_new,oneCollectionViewName_new] andInsertType:CCAddCodeType_Implementation toStrM:text insertFunction:nil];
            [self addCodeText:[NSString stringWithFormat:@"self.%@.delegate=self;\nself.%@.dataSource=self;",oneCollectionViewName,oneCollectionViewName] andInsertType:CCAddCodeType_InsertFunction toStrM:text insertFunction:@"- (void)viewDidLoad{"];
        }
        ...
        for (NSString *cell in cells) {
            NSString *adapterCell=[CCStroyBoardFileManager getAdapterCollectionViewCellName:cell];
