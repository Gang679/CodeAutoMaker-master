...
+ (void)saveDataHasTempFileWithData:(id)data WithIdentity:(NSString *)identity{
    ...
    //如果数据是字典
        ...
        NSString *RandFileName=[FilePath stringByAppendingString:[NSString stringWithFormat:@"%d.txt",(arc4random())]];
...
+ (void)insertDataWithData:(id)data WithIdentity:(NSString *)identity{
    ...
    if([data isKindOfClass:[NSString class]]){
        ...
        if (![[self getDataBase] executeUpdate:@"insert into CCJSONBLOB (DataType,Identity,JSONBLOBData) values (?,?,?)",@"NSDictionary",identity,myData]) {
            [self showAlertMessage:@"插入失败"];
        }
    }else if([data isKindOfClass:[NSArray class]]){
        NSArray *tempArr=(NSArray *)data;
        ...
        if (![[self getDataBase] executeUpdate:@"insert into CCJSONBLOB (DataType,Identity,JSONBLOBData) values (?,?,?)",@"NSArray",identity,myData]) {
            [self showAlertMessage:@"插入失败"];
        }
    }
}
...
+ (void)insertDataWithURL:(NSString *)URL WithIdentity:(NSString *)identity{
    ...
    if (![[self getDataBase] executeUpdate:@"insert into CCJSONBLOB (DataType,Identity,JSONBLOBData) values (?,?,?)",@"URL",identity,myData]) {
        [self showAlertMessage:@"插入失败"];
    }
}
