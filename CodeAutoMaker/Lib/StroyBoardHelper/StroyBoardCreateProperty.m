...
static NSMutableDictionary *CCStroyBoardCreateProperty;
static NSMutableDictionary *CCStroyBoardCreateCategory;
static NSMutableDictionary *CCStroyBoardCreateId;
static NSMutableDictionary *CCStroyBoardCreateFile;
static NSMutableDictionary *CCStroyBoardCreateContent;
...
+ (NSMutableDictionary *)defalutCreateProperty{
    ...
    dispatch_once(&onceToken, ^{
        if (CCStroyBoardCreateProperty==nil) {
            CCStroyBoardCreateProperty=[NSMutableDictionary dictionary];
        }
    });
    return CCStroyBoardCreateProperty;
}
+ (NSMutableDictionary *)defalutCreateCategory{
    //添加线程锁
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (CCStroyBoardCreateCategory==nil) {
            CCStroyBoardCreateCategory=[NSMutableDictionary dictionary];
        }
    });
    return CCStroyBoardCreateCategory;
}
+ (NSMutableDictionary *)defalutCreateFile{
    //添加线程锁
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (CCStroyBoardCreateFile==nil) {
            CCStroyBoardCreateFile=[NSMutableDictionary dictionary];
        }
    });
    return CCStroyBoardCreateFile;
}
+ (NSMutableDictionary *)defalutCreateContent{
    //添加线程锁
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (CCStroyBoardCreateContent==nil) {
            CCStroyBoardCreateContent=[NSMutableDictionary dictionary];
        }
    });
    return CCStroyBoardCreateContent;
}
+ (NSMutableDictionary *)defalutCreateId{
    //添加线程锁
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (CCStroyBoardCreateId==nil) {
            CCStroyBoardCreateId=[NSMutableDictionary dictionary];
        }
    });
    return CCStroyBoardCreateId;
}

+ (NSInteger)createPropertyWithStroyBoardPath:(NSString *)stroyBoardPath withProjectPath:(NSString *)projectPath{
    if ([CCFileManager fileExistsAtPath:stroyBoardPath]==NO||[CCFileManager fileExistsAtPath:projectPath]==NO) {
        return -1;
    }
    ...
    NSArray *files=[CCFileManager subPathFileArrInDirector:projectPath hasPathExtension:@[@".m"]];
    
    for (NSString *fileName in [self defalutCreateProperty]) {
        for (NSString *realFileName in files) {
            NSString *tempFileName=[CCFileManager getFileNameNoPathComponentFromFilePath:realFileName];
...
+ (NSString *)getCustomClassFromAllViews:(NSString *)text{
    ...
    for (NSInteger i=0; i<arr.count; i++) {
        ...
        viewIdenity=[CCStoryboardTextManager isView:rowStr];
...
//    @interface
    if ([strM rangeOfString:@"@interface"].location==NSNotFound) {
        NSString *interface=[NSString stringWithFormat:@"@interface %@ ()\n\
                             \n\
                             @end",fileName];
        [CCStoryboardTextManager addCodeText:interface andInsertType:CCAddCodeType_Import toStrM:strM insertFunction:nil];
    }
    
    for (NSString *property in propertys) {
        NSString *newProperty=[self getPropertyCode:property];
        if ([strM rangeOfString:newProperty].location==NSNotFound) {
            [CCStoryboardTextManager addCodeText:newProperty andInsertType:CCAddCodeType_Interface toStrM:strM insertFunction:nil];
        }
    }
    ...
    if(propertys.count>0){
        if ([strM rangeOfString:@"- (void)setPropertyValues{"].location!=NSNotFound) {
            ...
            [CCStoryboardTextManager addCodeText:setValue andInsertType:CCAddCodeType_InsertFunction toStrM:strM insertFunction:@"- (void)setPropertyValues{"];
        }else{
            NSString *setValue=[self getSetValueFunc:propertys isExsitFunc:NO];
            [CCStoryboardTextManager addCodeText:setValue andInsertType:CCAddCodeType_Implementation toStrM:strM insertFunction:nil];
        }
    }
}
...
+ (NSString *)getPropertyCode:(NSString *)property{
    ...
    category=[CCNSString removeSpaceBeforeAndAfterWithString:category];
    ...
    viewCategory=[@"UI" stringByAppendingString:[CCStoryboardTextManager upFirstCharacter:category]];
...
+ (void)backupNewStroyBoard:(NSString *)filePath{
    //有后缀的文件名
    NSString *tempFileName=[CCFileManager getFileNameFromFilePath:filePath];
    
    //无后缀的文件名
    NSString *fileName=[CCFileManager getFileNameNoPathComponentFromFilePath:filePath];
    ...
    [CCFileManager copyItemAtPath:filePath toPath:newFilePath];
}
