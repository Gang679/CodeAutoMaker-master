...
static NSMutableDictionary *CCStroyBoardFileDicM;
static NSMutableDictionary *CCStroyBoardContextDicM;
static NSString *MainDirectory;
@implementation CCStroyBoardFileManager
+ (NSMutableDictionary *)defalutFileDicM{
    //添加线程锁
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (CCStroyBoardFileDicM==nil) {
            CCStroyBoardFileDicM=[NSMutableDictionary dictionary];
        }
    });
    return CCStroyBoardFileDicM;
}
+ (NSMutableDictionary *)defalutContextDicM{
    //添加线程锁
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        if (CCStroyBoardContextDicM==nil) {
            CCStroyBoardContextDicM=[NSMutableDictionary dictionary];
        }
    });
    return CCStroyBoardContextDicM;
}
...
+(void)done{
    ...
    for (NSString *filePath in [self defalutContextDicM]) {
        ...
        context=[NSMutableString stringWithString:[[CCWordWrap new] wordWrapText:context]];
...
//    NSLog(@"%@",[self defalutFileDicM]);
    ...
    MainDirectory=nil;
    [CCStroyBoardContextDicM removeAllObjects];
    [CCStroyBoardFileDicM removeAllObjects];
}
...
//    [[CCWordWrap new]wordWrap:directory];
...
//    text=[[CCWordWrap new]wordWrapText:text];//为了便于CCAddCode好寻找位置,这里先对其进行代码缩进处理
